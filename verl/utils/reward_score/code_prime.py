# https://github.com/PRIME-RL/PRIME/blob/main/training/verl/utils/reward_score/evaluation_utils/code_util/__init__.py
...
def apps_check_correctness(in_outs: Optional[dict], generation, timeout=10, debug=True):
...
def evaluate_code(completion, test_cases):
    ...
    try:
        ...
        # First check all test cases together
        try:
            ...
            if all(map(lambda x: x == True, res)):
                return 1.0  # All test cases passed
        except Exception as e:
            pass

        # If bulk check fails, check test cases one by one
        test_cases_list = []
        ...
        for test_case_id, test_case in enumerate(test_cases_list):
            ...
            try:
                metadata = dict(enumerate(metadata))[0]  # metadata might be empty if computation fails
            except Exception as e:
                metadata = {}
            metadata["test_case"] = {
        ...
        # Calculate score using actual number of processed test cases
        passing_tests = sum(1 for x in res_list if x == True)
        num_test_cases = len(res_list)  # Use actual number of processed test cases
        score = passing_tests / num_test_cases if num_test_cases > 0 else 0.0
        return score

    except Exception as e:
        traceback.print_exc(10)
        return 0.0  # Return 0.0 if there's any error in evaluation
...
async def _timeout_wrapper(func: Callable, args: tuple, timeout: int) -> float:
    try:
...
def compute_score(solution_str, ground_truth) -> float:
    ...
    return timeout_wrapper(evaluate_code, args=(solution_str, ground_truth), timeout=300)